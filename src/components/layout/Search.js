...
class Search extends Component {
  ...
  updateHeading = (search_code) => {
    if (search_code === "M") {
      return (
        <h2>
          Search results for:{" "}
          <span className="color-M">{this.state.title}</span>
        </h2>
      );
      ...
      return (
        <h2>
          Search results for:{" "}
          <span className="color-G">{this.state.title}</span>
        </h2>
      );
    } else {
      return (
        <h2>
          Search results for:{" "}
          <span className="color-B">{this.state.title}</span>
        </h2>
      );
    }
  };
  ...
  findMedia = (e, updateMedias) => {
    ...
    if (this.state.media === "Movie") {
      axios
        .get(
          `https://api.themoviedb.org/3/search/movie?api_key=${process.env.REACT_APP_TMDB_KEY}&language=en-US&query=${this.state.title}&page=1&include_adult=false`
        )
        .then((res) => {
      ...
      axios
        .get(
          `https://cors-anywhere.herokuapp.com/https://www.giantbomb.com/api/search/?api_key=${process.env.REACT_APP_GIANTBOMB_KEY}&format=json&query=${this.state.title}&resources=game`
        )
        .then((res) => {
          // console.log(res.data.results)
      ...
      axios
        .get(
          `https://reststop.randomhouse.com/resources/titles?search=${this.state.title}`
        )
        .then((res) => {
                ...
                <form onSubmit={(e) => this.findMedia(e, updateMedias)}>
                  <div className="form-row align-items-center">
                    <div className="col-md-3 my-1">
                      <label className="mr-sm-2 sr-only" htmlFor="formSelect">
                        Preference
                      </label>
                      <select
                    ...
                    <div className="col-md-6 my-1">
                      <input
                        ...
                        placeholder="Movie, game or book title..."
                        name="title"
                    ...
                    <div className="col-md-3 my-1">
                      <button
                        className="btn btn-lg btn-block search-btn"
                        type="submit"
                      >
                        Submit
                      </button>
